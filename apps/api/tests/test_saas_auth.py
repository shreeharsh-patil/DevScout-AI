...
class TestSaaSAuthAndIsolation:
    ...
    def test_cross_tenant_report_isolation_and_403_forbidden(self):
        ...
        res_a = client.get(
            f"/api/v1/research/report/{report_a.job_id}",
        ...
        assert not any(item["job_id"] == report_a.job_id for item in hist_b)
    ...
    def test_saved_rename_and_delete_lifecycle(self):
        ...
        # 3. Delete report
        res_del = client.delete(f"/api/v1/reports/{job.job_id}", headers=headers)
        assert res_del.status_code == 200

        # 4. Confirm deleted
        res_after = client.get(f"/api/v1/research/status/{job.job_id}", headers=headers)
    ...
    def test_credit_deduction_and_usage_tracking(self):
        ...
        res_usage = client.get(f"/api/v1/workspaces/{ws.id}/usage", headers=headers)
